...
const initialState: ICounterState = { num: 0, loadingCount: 0 }

export default function reducer(state: ICounterState = initialState, action: IMyAction): ICounterState {
  switch (action.type) {
  ...
  public async fetchAmount(): Promise<void> {
    ...
    try {
      ...
      if (response.status === 200) { // 2xx
        const json = await response.json<IJsonObject>()
